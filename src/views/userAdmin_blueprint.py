                ...
                if books_url:
                    ...
                    for i in books_url:
                        ...
                        if latest_data:
                            ...
                            item_result['quickreading_content_url'] = get_latest_data.get('quickreading_content_url', '')
                        item_result['add_time'] = i.get('add_time', '')
                        ...
                        result.append(item_result)
                    return template('admin_books.html', title='bookshelf of {user}'.format(user=user),
                                    is_login=1,
                                    user=user,
                                    is_bookmark=1,
                                    result=result[::-1])
            return template('admin_books.html', title='bookshelf of {user}的书架'.format(user=user),
                            is_login=1,
                            user=user,
                            is_bookmark=0)
        except Exception as e:
